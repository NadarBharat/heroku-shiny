...
library(stringi)
                         ...
                         tabItem(tabName = "lda",
                                 fluidPage(
                                   wellPanel(
                                     ...
                                     numericInput("nTopics", value = 7, min = 3, max = 500, label = tags$span(
                                       "Number of Topics", 
                                       tags$i(
                                     ...
                                     numericInput("alpha", value = 0.1, min = 0, max = 1, label = tags$span(
                                       "Alpha Value", 
                                       tags$i(
                                         class = "glyphicon glyphicon-info-sign", 
                                         style = "color:#0072B2;",
                                     ...
                                     numericInput("eta", value = 0.01, min = 0, max = 1, label = tags$span(
                                       "Eta Value", 
                                       tags$i(
                                         class = "glyphicon glyphicon-info-sign", 
                                         style = "color:#0072B2;",
                                     ...
                                     sliderInput("nIteration", min = 500, max = 5000, value = 1000, step=500, label = tags$span(
                                       "Iteration Value", 
                                       tags$i(
                                         class = "glyphicon glyphicon-info-sign", 
                                         style = "color:#0072B2;",
                                     ...
                                     tags$hr(),
                                     numericInput("percentcf", min = 0.01, max = 0.99, value = 0.25, label = tags$span(
                                       "Percent Cut Off", 
                                       tags$i(
                                         class = "glyphicon glyphicon-info-sign", 
                                         style = "color:#0072B2;",
                                     ...
                                     tags$hr(),
                                     downloadLink("coherenceoutput", "Coherence Score")
                                   ),
                                   mainPanel(
                                     withSpinner(visOutput('visChart'), type = 6)
                                   )
                                 )),
                         ...
                         tabItem(tabName = "lsa",
                                 fluidPage(
                                   wellPanel(
                                     ...
                                     numericInput("nTopics_lsa", value = 7, min = 3, max = 500, 
                                                  label = tags$span(
                                                    "Number of Topics", 
                                                    tags$i(
                                                      class = "glyphicon glyphicon-info-sign",
                                     ...
                                     numericInput("percentcf_lsa", min = 0.01, max = 0.99, value = 0.25, 
                                                  label = tags$span(
                                                    "Percent Cutoff", 
                                   ...
                                   mainPanel(
                                     fluidRow(
                                       column(12,
                                              downloadLink("LSAoutput", "Download Your File Here...")
                                       ),
                                       fluidRow(column(12,
                                                       withSpinner(dataTableOutput("LSAdata"), type = 6))))
                                   )
                                 )
                         ),
  ...
  server = function(input, output, session) {
    ...
    coherence = reactive({
      LDAresult = LDAresult()
      pagesLDA = pagesLDA()
      coherence = topicCoherence(ldaresult=LDAresult, documents=pagesLDA, num.words=5, by.score=TRUE)
      coherence = as.data.frame(t(coherence))
    })
    
    
    output$coherenceoutput = downloadHandler(
      filename = function() {
        paste("Coherence Score", ".csv", sep = "")
      },
      content = function(file){
        ...
        coherenceoutput = coherence()
        write.csv(coherenceoutput, file)
      })
    
    #output$coherence = renderText(return(coherence()))
    ...
    observe({
      choices1 = colnames(datafile_org())
      updateSelectInput(session,"ID_lsa", choices =  choices1, selected = "id")
    })
    
    observe({
      choices1 = colnames(datafile_org())
      updateSelectInput(session,"Text_lsa", choices =  choices1, selected = "text")
    })
    # DTM
    dtm = reactive({
      datatable = datafile_org()
      ...
      dtm = CreateDtm(doc_vec = datatable$cleantext, # character vector of documents
                      doc_names = datatable[,id], # document names
    ...
    doc_with_topic_lsa = reactive({
      ...
      LSAresult$labels = LabelTopics(assignments = LSAresult$theta > 0.05, 
                                     dtm = dtm,
                                     M = 1)
      
      # put them together, with coherence into a summary table
      LSAresult$summary = data.frame(topic = rownames(LSAresult$phi),
    ...
    observeEvent(input$datafile, {
      ...
      output$LSAdata = renderDataTable({
        datatable(doc_with_topic_lsa(), 
                  options = list(
                    autoWidth = TRUE,
                    scrollY = TRUE
                  ))
